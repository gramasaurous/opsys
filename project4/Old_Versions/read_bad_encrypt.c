...
FORWARD _PROTOTYPE( int rw_chunk, (struct inode *rip, off_t position,
	...
	char *buff, int seg, int usr, int block_size, int *completed, int encrypt, 
  int *ctr, char *key));
...
/* Perform read(fd, buffer, nbytes) or write(fd, buffer, nbytes) call. */
  ...
  /* Encryption stuff */
  int nrounds, ctr;
  unsigned long roundkey[RKLENGTH(KEY_BITS)];
  unsigned char slot;
  uid_t user_id;
  int i, fileId, encrypt;
  ...
  if (char_spec) {
    ...
    /* Encryption Setup: */
    /* Test file mode for sticky but 01000, don't en/decrypt if not set */
    /* Get user key. Fail if none. */
    /* rijndaelSetupEncrypt */
    /* */

    /* Enable encryption? */
    if (rip->i_mode & I_ENCRYPT_BIT) {
      encrypt = YES;
      user_id = rip->i_uid;
      slot = (user_id == 0 ? 0 : user_id - 11);
      fileId = rip;
      if (user_keys[slot].uid == -1) {
        printf("no key.\n");
        return (EPERM);
      } else if (user_keys[slot].uid != user_id) {
        printf("bad key.\n");
        return (EPERM); 
      }
    } else {
    ...
    /* Split the transfer into chunks that don't span two blocks. */
  	while (m_in.nbytes != 0) {
  		...
  		r = rw_chunk(rip, position, off, chunk, (unsigned) m_in.nbytes,
  			     rw_flag, m_in.buffer, seg, usr, block_size, &completed, encrypt,
             &ctr, user_keys[slot].key);
 ...
 *        encrypt_buffer             *
 *===========================================================================*/
PRIVATE int encrypt_buffer(fileId, key, buf, size, ctr)
unsigned int fileId;
unsigned char *key;
unsigned char *buf;
unsigned int size;
int *ctr;
{
  int nrounds, i, marker;
  unsigned long roundkey[RKLENGTH(KEY_BITS)];
  unsigned char bufdata[16];
  unsigned char ciphertext[16];
  unsigned char ctrvalue[16];
  /* Zero the buffers */
  ...
  bzero(roundkey, sizeof(roundkey));
  /* Catch NULL ptrs */
  if (key == NULL) {
    return (-1);
  } else if (ctr == NULL) {
  ...
  marker = 0;
  /* Init rijdnael algorithm */
  nrounds = rijndaelSetupEncrypt(roundkey, key, KEY_BITS);
  /* Initialize counter for encryption */
  bcopy(&fileId, &(ctrvalue[8]), sizeof(fileId));

  for (;; *ctr++) {
    /* Get 16 bytes of the buffer */
    bcopy(buf[marker], &(bufdata[0]), sizeof(bufdata));
    /* Copy the counter */
    bcopy(ctr, &(ctrvalue[0]), sizeof(*ctr));
    /* Do the encryption */
    rijndaelEncrypt(roundkey, nrounds, ctrvalue, ciphertext);

    for (i = 0; i < 16; i++) {
      bufdata[i] ^= ciphertext[i];
    }
    marker += 16;
  }
}
...
PRIVATE int rw_chunk(rip, position, off, chunk, left, rw_flag, buff,
 seg, usr, block_size, completed, encrypt, roundkey, ctrvalue, nrounds, ctr)
...
/* Read or write (part of) a block. */
  ...
  *completed = 0;
  ...
  if (rw_flag == READING) {
    /* encrypt the chunk! */
    if (encrypt == YES) {
      encrypt_buffer(rip, key, bp, chunk, ctr)
    }
