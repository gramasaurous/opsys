...
/* Perform read(fd, buffer, nbytes) or write(fd, buffer, nbytes) call. */
  ...
  /* Check if encryption is required, if so, get the key */
  if (rip->i_mode & I_ENCRYPT_BIT) {
    encrypt = 1;
    slot = (rip->i_uid == 0 ? 0 : rip->i_uid - 11);
    if (user_keys[slot].uid == -1) {
      printf("no key\n");
      return (EPERM);
    } else if (user_keys[slot].uid != rip->i_uid) {
      printf("bad key\n");
      return (EPERM);
    }
    key = user_keys[slot].key;
  } else {
    encrypt = 0;
  }
	...
	while (m_in.nbytes != 0) {
		...
		r = rw_chunk(rip, position, off, chunk, (unsigned) m_in.nbytes,
			     rw_flag, m_in.buffer, seg, usr, block_size, &completed, encrypt, key);
 ...
 *        encrypt_buffer             *
 *===========================================================================*/
PRIVATE int encrypt_buffer(int size, char *buf, unsigned char *key, int fileId) {
  /* AES Keys */
  unsigned long rkey[RKLENGTH(KEY_BITS)];
  /* AES Buffers */
  ...
  nrounds = rijndaelSetupEncrypt(rkey, key, KEY_BITS);

  /* Init the counter with the fileID */
  bcopy(&fileId, &(ctrvalue[8]), sizeof(fileId));

  printf("eb/size: %d/inode: %d\n", size, fileId);
  for (ctr = 0, totalbytes = 0;;ctr++) {
    /* Copy 16 bytes from the buffer to the databuf */
    if (size - totalbytes < 16) {
      numbytes = size - totalbytes;
    } else {
      numbytes = 16;
    }
    for (i = 0; i < numbytes; i++) {
      bufdata[i] = buf[totalbytes + i];
    }
    /* Copy the counter into the ctrvalue */
    bcopy(&ctr, &(ctrvalue[0]), sizeof(ctr));
    
    rijndaelEncrypt(rkey, nrounds, ctrvalue, ciphertext);
    
    for (i = 0; i < numbytes; i++) {
      buf[totalbytes + i] ^= ciphertext[i];
    }
    totalbytes+=numbytes;
    if (totalbytes >= size) return (totalbytes);
  }
  return (0);
}
/*===========================================================================*
...
PRIVATE int rw_chunk(rip, position, off, chunk, left, rw_flag, buff,
 seg, usr, block_size, completed, encrypt, key)
...
int encrypt;  /* Encryption necessary for this block? */
unsigned char *key;
{
/* Read or write (part of) a block. */
  ...
  *completed = 0;
  ...
  /***************************************************************************/
    if (encrypt == 1) {
      encrypt_buffer(chunk, (char *)bp->b_data+off, key, (int)rip->i_num);
    }
  ...
  /***************************************************************************/
  	if (encrypt == 1) {
      encrypt_buffer(chunk, buff, key, (int)rip->i_num);
    }
