...
PUBLIC int do_setkey() {
  ...
  int i,j, slot;
    /* Check for unnaceptable user_ids */
  if ((uid > 0 && uid < 12) || uid > 18) {
    return (-1);
  }
    /* Find the slot */
    /* slot 0 reserved for uid=0, 1-7 for uid=12-18 */
  if (uid == 0) {
    slot = 0;
  } else {
    slot = (uid - 11);
  ...
  if (k0 == 0 && k1 == 0) {
    user_keys[slot].uid = -1; 
  } else {
    user_keys[slot].uid = uid;
  }
  
  bcopy (&k0, &(user_keys[slot].key[0]), sizeof (k0));
  bcopy (&k1, &(user_keys[slot].key[sizeof(k0)]), sizeof (k1));
